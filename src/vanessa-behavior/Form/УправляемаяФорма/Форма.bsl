...
&НаКлиенте
&НаКлиенте
Функция ПолучитьСтрокуВызоваSikuli(ФайлОтладки) Экспорт
	Если ПустаяСтрока(ФайлОтладки) Тогда
		Возврат "call """ + Объект.КаталогИнструментов + "\tools\Sikulix\runsikulix"" -r";
	Иначе
		Возврат "call """ + Объект.КаталогИнструментов + "\tools\Sikulix\runsikulix"" -d 3 -f """ + ФайлОтладки + """ -r";
	КонецЕсли;
КонецФункции	
...
&НаКлиенте
Функция ВыполнитьSikuliСкрипт(СтрокаКоманды,ЖдатьОкончания = -1, ФайлОтладки = "") Экспорт
	Стр = ПолучитьСтрокуВызоваSikuli(ФайлОтладки) + " " + СтрокаКоманды;
